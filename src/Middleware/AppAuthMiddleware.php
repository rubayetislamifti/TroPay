...
//        dd(Request::header('X-App-Key'), Request::header('X-App-Secret'));
        ...
        $info = DB::table('api_clients')->where('live_app_key', $appKey)->where('live_app_secret',$appSecret)
            ->orWhere('sandbox_app_key', $appKey)->where('sandbox_app_secret', $appSecret)->first();
